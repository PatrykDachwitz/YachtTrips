<?php

return [
    "menu" => [
        'files_manager' => "Menadżer plików",
        'orders' => "Zamówienia",
        'oceans' => "Oceany",
        'trips' => "Podróże",
        'yachts' => "Jachty",
        'rooms' => "Pokoje",
        'pages' => "Strony",
        'banners' => "Bannery",
        'sales' => "Sprzedaż",
        'travels' => "Wycieczki",
        'content' => "Zawartość",
        'users' => "Użytkownicy",
        'logOut' => "Wyloguj się",
        'vacationers' => "vacationers",
    ],
    'list' => [
        "banners" => "Lista banerów",
        'bookings' => "Lista rezerwacji",
        'categoryBanners' => "Lista kategori bannerów",
        'methodPayments' => "Lista metod płatności",
        'oceans' => "Lista oceanów",
        'orders' => "Lista zamówień",
        'pages' => "Lista stron",
        'rooms' => "Lista pokoji",
        'templates' => "Lista szablonów",
        'trips' => "Lista wakacji",
        'users' => "Lista użytkowników",
        'vacationers' => "Lista podróżników",
        'yachts' => "Lista jachtów",
    ],
    'new' => [
        "banners" => "Nowy baner",
        'bookings' => "Nowa rezerwacja",
        'categoryBanners' => "Nowa kategoria bannerów",
        'methodPayments' => "Nowa metoda płatności",
        'oceans' => "Nowy ocean",
        'orders' => "Nowe zamówienie",
        'pages' => "Nowa strona",
        'rooms' => "Nowy pokój",
        'templates' => "Nowy szablon",
        'trips' => "Nowe wakacje",
        'users' => "Nowy użytkownik",
        'vacationers' => "Nowy podróżni",
        'yachts' => "Nowy jacht",
    ],
    'views' => [
        'view' => 'Wyświetl',
        'edit' => 'Edytuj',
        'delete' => 'Usuń',
    ],
    'name' => 'Nazwa',
    'view' => 'Wyświetl',
    'add' => 'Dodaj',
    'edit' => 'Edytuj',
    'active' => "Aktywne",
    'category' => "Kategoria",
    'startView' => "Rozpoczęcie wyświetlania",
    'endView' => "Koniec wyświetlania",
    'yes' => "Tak",
    'no' => "Nie",
    'template' => "Szablon",
    'templates' => "Szablony",
    'banner' => "Banner",
    'categoriesBanner' => "Kategoria banerów",
    'oceans' => "Oceany",
    'trips' => "Podróże",
    'yachts' => "Jachty",
    'newTemplate' => "Dodaj szablon",
    'newBanner' => "Dodaj banner",
    'newOceans' => "Dodaj oceany",
    'newTrips' => "Dodaj podróże",
    'newYachts' => "Dodaj jachty",
    'listTemplate' => "Lista szablonów",
    'listBanner' => "Lista bannerów",
    'listOceans' => "Lista oceanów",
    'listTrips' => "Lista podróży",
    'listYachts' => "Lista jachtów",
    'availablePlace' => "Dostępne miejsca",
    'startDay' => "Pierwszy dzień",
    'endDay' => "Ostatni dzień",
    'countDay' => "Ilość dni",
    'ocean' => "Ocean",
    'yacht' => "Jacht",
    'order' => "Zamówienia",
    'booking' => "Rezerwacje",
    'client' => "Klienci",
    'payments' => "Metody płatności",
    'sale' => "Sprzedaż",
    'travels' => "Wycieczki",
    'content' => "Zawartość",
    'users' => "Użytkownicy",
    'files_manager' => "Menadżer plików",
    'logOut' => "Wyloguj się",
    'rooms' => "Pokoje",
    'pages' => "Strony",
    'trip_id' => "Numer wycieczki",
    'order_id' => "Numer zamówienia",
    'number_room' => "Numer pokoju",
    'count_adult' => "Osoby dorosłe",
    'count_kids' => "Osoby niepełnoletnie",
    'double_beds' => "Podwójne łóżka",
    'single_beds' => "Pojedyncze łózka",
    'kids_beds' => "Dziecięce łózka",
    'price' => "Cena",
    'correspondenceAddress' => "Adres korespondencji",
    'address' => "Adres",
    'personal_last_name' => "Nazwisko",
    'personal_first_name' => "Imię",
    'personal_phone' => "Numer telefonu",
    'personal_mail' => "Adres e-mail",
    'slug' => "Slug",
    'session_id' => "Numer sesji",
    'status' => "Status",
    'superAdmin' => "Status super admina",
    'passwordConfirmed' => "Powtórz hasło",
    'password' => "Hasło",
    'security' => "Stopień dostępu",
    'premium' => "Premium",
    'checkedRules' => "Zgoda z regulaminem",
    'typePayment' => "Typ płatności",
    'age' => "Wiek",
    'bookingId' => "Numer rezerwacji",
    'adult' => "Osoba dorosła",
    'kid' => "Osboba niepełnoletnia",
    'tripId' => "Pokój id",
    'cost' => "Koszt",
    'available' => "Dostępny",
    'true' => "Tak",
    'false' => "Nie",
];
